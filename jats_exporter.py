...
def _parse_affiliations_txt(afiliaciones_txt: str) -> tuple[list[dict], list[str]]:
    ...
    La etiqueta visible puede conservar el numero original del txt.
    """
    afils: list[dict] = []
    emails: list[str] = []
    id_seq = 1
    for raw in (afiliaciones_txt or "").splitlines():
        ...
        if m:
            num, resto = m.group(1), _clean_text(m.group(2))
            afils.append({"id": f"aff{id_seq}", "label": num, "text": resto})
            id_seq += 1
        else:
